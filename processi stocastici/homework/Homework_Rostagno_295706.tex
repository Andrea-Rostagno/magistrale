...
\begin{document}
	...
	\begin{itemize}
		...
		\begin{enumerate}[label=\alph*)]
			...
			\item $P_1(	U \leq 6)=P(U=1)+P(U=1)+P(U=2)+P(U=3)+P(U=4)+P(U=5)+P(U=6)$ dove ogni probabilità $P(U=k)=P(\text{la catena sia in }\{1,2,3\} \text{al passo k} ) \cdot \\P(\text{la catena sia in \{4,5\} al passo k+1})$.\\
